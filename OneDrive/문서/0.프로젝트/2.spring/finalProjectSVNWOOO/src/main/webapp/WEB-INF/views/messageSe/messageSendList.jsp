		...
		<table>
			...
			<tbody>
				<c:forEach items="${messageList}" var="message">
					<tr>
						<td><c:out value="${message.m_id}" /></td>
						<td><a class='move' href='<c:out value="${message.m_id}"/>'>
								<c:out value="${message.m_title}" />
						</a></td>

						<td data-cell-header="신청일자"><fmt:formatDate
								pattern="yyyy-MM-dd HH:mm" value="${message.m_time}" /></td>
						<td><c:out value="${message.m_eid1}" /></td>
						<td><c:out value="${message.m_sc}" /></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
...
<div class="brd_btn_control right mt30">
	<button class="btn_sz_md btn_bg1" type=button
		onclick="location.href='/messageSe/send'">쪽지 보내기</button>
</div>
<!-- paging
...
<form id='actionForm' action="/message/messageSendList" method='get'>
...
<script type="text/javascript">
   $(document).ready(function(){
      ...
      function checkModal(result){
         ...
         actionForm.attr("action","/messageSe/messageSendGet");
         actionForm.submit();
